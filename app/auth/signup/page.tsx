  ...
  useEffect(() => {
    const checkAccess = async () => {
      try {
        ...
        // If public registration is not allowed, redirect to signin
        if (!data.allowPublicRegistration) {
          router.replace('/auth/signin')
          return
        }
      } catch (error) {
        console.error('Error checking access:', error)
        // On error, redirect to signin as safe default
        router.replace('/auth/signin')
      } finally {
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result?.error) {
        ...
        // New users have no businesses, redirect to onboarding
