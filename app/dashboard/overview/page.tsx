...
export default function OverviewPage() {
  ...
  const [showAllBrandsDialog, setShowAllBrandsDialog] = useState(false);
  ...
  // Calculate displayed brands (8 max, with business always visible)
  const displayedBrandRankings = useMemo(() => {
    if (brandRankings.length <= 8) return brandRankings;

    const businessName = business?.businessName;
    const businessIndex = brandRankings.findIndex(b => b.name === businessName);

    // If business is in top 8, just return top 8
    if (businessIndex >= 0 && businessIndex < 8) {
      return brandRankings.slice(0, 8);
    }

    // Business is not in top 8, show top 7 + business at its actual position
    const top7 = brandRankings.slice(0, 7);
    if (businessIndex >= 0) {
      return [...top7, brandRankings[businessIndex]];
    }

    // No business found, just return top 8
    return brandRankings.slice(0, 8);
  }, [brandRankings, business?.businessName]);

  // Memoize the filtered brands list - only show brands that are in the displayed table AND selected
  const visibleBrands = useMemo(() => {
    const displayedNames = new Set(displayedBrandRankings.map(b => b.name));
    return allBrands.filter(brand => selectedBrands.has(brand) && displayedNames.has(brand));
  }, [allBrands, selectedBrands, displayedBrandRankings]);
    ...
    <div>
        ...
        <div className={`mb-4 ${isRefreshing ? 'opacity-50' : ''}`}>
          ...
          <div className="hidden lg:flex gap-4 mb-2">
            ...
            <div className="flex-[1_1_50%] min-w-0 flex justify-between items-center">
              ...
              <div className="flex items-center gap-3">
                {brandRankings.length > 8 && (
                  <button
                    onClick={() => setShowAllBrandsDialog(true)}
          ...
          <div className="flex gap-4 flex-col lg:flex-row lg:items-stretch">
            ...
            <div className="flex-[1_1_50%] min-w-0 flex flex-col">
              ...
              <Card>
                ...
                <div>
                  <Table>
                    ...
                    <TableBody>
                      {displayedBrandRankings.map((brand) => {
                        ...
                        // Get actual rank in full list for display
                        const actualRank = brandRankings.findIndex(b => b.name === brand.name) + 1;
        ...
        {/* All Brands Dialog */}
        <Dialog open={showAllBrandsDialog} onOpenChange={setShowAllBrandsDialog}>
          <DialogContent className="max-w-3xl max-h-[80vh] overflow-hidden flex flex-col">
            <DialogHeader>
              <DialogTitle>Brand Rankings</DialogTitle>
              <DialogDescription>
                All {brandRankings.length} brands ranked by visibility
              </DialogDescription>
            </DialogHeader>
                ...
                <TableBody>
                  {brandRankings.map((brand, index) => {
                    ...
                    return (
                      <TableRow
                        key={brand.name}
                        className={`cursor-pointer transition-all hover:bg-gray-100 ${isSelected ? 'opacity-100' : 'opacity-40'} ${brand.name === business?.businessName ? 'bg-blue-50' : ''}`}
